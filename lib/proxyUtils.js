...
export function rewriteM3U8(m3u8Content, baseUrl, headers, isMaster = false) {
        ...
        if (line.startsWith('#')) {
            ...
            if (line.startsWith('#EXT-X-KEY:')) {
                ...
                if (uriMatch) {
                    ...
                    const proxyKeyUrl = `${web_server_url}/api/ts-proxy?url=${encodeURIComponent(keyUrl)}&headers=${encodeURIComponent(JSON.stringify(headers))}`;
            ...
            // Handle media playlists in master playlist
                ...
                if (uriMatch) {
                    ...
                    const proxyMediaUrl = `${web_server_url}/api/proxy?url=${encodeURIComponent(mediaUrl)}&headers=${encodeURIComponent(JSON.stringify(headers))}`;
            ...
            try {
                ...
                const endpoint = isMaster ? '/api/proxy' : '/api/ts-proxy';
                const proxyUrl = `${web_server_url}${endpoint}?url=${encodeURIComponent(segmentUrl)}&headers=${encodeURIComponent(JSON.stringify(headers))}`;
